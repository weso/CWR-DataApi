...
from cwr.parser.encoder.dictionary import AgreementEncoder
...
class TestAgreementRecordDictionaryEncoding(unittest.TestCase):
    def setUp(self):
        self._encoder = AgreementEncoder()
    ...
    def test_encoded(self):
        ...
        self.assertEqual(datetime.datetime.strptime('20030215', '%Y%m%d').date(), encoded['agreement_start_date'])
        ...
        self.assertEqual(datetime.datetime.strptime('20030216', '%Y%m%d').date(), encoded['agreement_end_date'])
        self.assertEqual(datetime.datetime.strptime('20030217', '%Y%m%d').date(), encoded['date_of_signature'])
