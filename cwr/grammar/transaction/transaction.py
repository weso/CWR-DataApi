...
from cwr.grammar.record import publisher, non_roman_alphabet, writer, agreement, work, ari
...
work_transaction = work.work_record + pp.Optional(pp.OneOrMore(interested_party.controlled_publisher_information)) + \
                   ...
                   pp.Optional(pp.OneOrMore(rule_work_detail.alternate)) + pp.Optional(non_roman_alphabet.nat) + \
...
# Acknowledgement
acknowledgement_transaction = _factory_record.get_transaction_record('acknowledgement') + pp.Optional(pp.OneOrMore(_factory_record.get_transaction_record('message'))) + (
